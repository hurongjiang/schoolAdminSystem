...
def export_class_info_excel(request,schoolrool):
    # 设置HTTPResponse的类型
    response = HttpResponse(content_type='application/vnd.ms-excel')
    class_bj = schoolrool.first().class_bj.class_bj
    grade = schoolrool.first().grade.grade
    file_name = '%s%s%s%s'%(request.user.username,grade,class_bj,datetime.datetime.now())
    response['Content-Disposition'] = 'attachment;filename=%s.xls'%urlquote(file_name)
    output = export_class_info_writer(request,schoolrool)
    response.write(output.getvalue())
    return response

def export_class_info_writer(request,schoolrool):
    xldata = xlwt.Workbook(encoding='utf8')
    class_bj = schoolrool.first().class_bj.class_bj
    grade = schoolrool.first().grade.grade
    sheet_name = '%s%s%s'%(request.user.username,grade,class_bj)
    sheet = xldata.add_sheet(sheet_name)
    style_heading = xlwt.easyxf("""
            font:
                name Arial,
                colour_index white,
...
def export_grade_info_excel(request,schoolrool):
    # 设置HTTPResponse的类型
    response = HttpResponse(content_type='application/vnd.ms-excel')
    grade = schoolrool.first().grade.grade
    file_name = '%s%s%s'%(request.user.username,grade,datetime.datetime.now())
    response['Content-Disposition'] = 'attachment;filename=%s.xls'%urlquote(file_name)
    output = export_grade_info_writer(request,schoolrool)
    response.write(output.getvalue())
    return response

def export_grade_info_writer(request,schoolrool):
    xldata = xlwt.Workbook(encoding='utf8')
    class_bj = schoolrool.first().class_bj.class_bj
    grade = schoolrool.first().grade.grade
    sheet_name = '%s%s'%(request.user.username,grade)
    sheet = xldata.add_sheet(sheet_name)
    style_heading = xlwt.easyxf("""
            font:
                name Arial,
